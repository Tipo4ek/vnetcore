...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "VNETNetwork") type = VNETNetwork;
    if(strType == "VNETNetworkVariable") type = VNETNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == VNETNetwork) s = "VNETNetwork";
    if(type == VNETNetworkVariable) s = "VNETNetworkVariable";
