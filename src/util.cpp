...
const char * const BITCOIN_CONF_FILENAME = "vnet.conf";
const char * const BITCOIN_PID_FILENAME = "vnetd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "vnet" is a composite category enabling all vnet-related debug output
            if(ptrCategory->count(string("vnet"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\vnet
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\vnet
    // Mac: ~/Library/Application Support/vnet
    // Unix: ~/.vnet
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VNet";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/VNet";
#else
    // Unix
    return pathRet / ".vnet";
#endif
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty vnet.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override vnet.conf
