...
#if defined(NDEBUG)
# error "VNet Core cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in vnets) are considered zero fee (for relaying, mining and transaction creation) */
  ...
  int halvings = nHeight / consensusParams.nSubsidyHalvingInterval;
    ...
    if (halvings >= 35)
        return 0;
    		...
    		if (nHeight == 0)
    			return 400000 * COIN; // Dev fund
           else if (nHeight == 1)
              		return 100000 * COIN;  // Bounty Fund
    		if (nHeight >= 2 && nHeight <= 150) // instamine protection
    			return 1 * COIN;

      //printf("height %u diff %4.2f reward %d\n", nHeight, dDiff, nSubsidyBase);
      CAmount nSubsidy = 3 * COIN;

      // Subsidy is cut in half every 51,840 blocks which will occur approximately.
      nSubsidy >>= halvings;
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    int nMNPIPeriod = Params().GetConsensus().nMasternodePaymentsIncreasePeriod;
                                                                      ...
                                                                      // mainnet:
    if(nHeight > nMNPIBlock)                  ret += blockValue; // 50 to start
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // VNET : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(VNET): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(VNET): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // VNET : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(VNET): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(VNET): spork is off, skipping transaction locking checks\n");
    }
